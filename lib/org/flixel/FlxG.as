package org.flixel
{
	...
	public class FlxG
	{
		 ...
		 * Loads a bitmap from a file, caches it, and generates a horizontally flipped version if necessary.
		 * 
		 * @param	Source		The BitmapData you want to dispose from the cache.
		 * @param	Reverse		Whether to generate a flipped version.
		 */
		static public function disposeBitmapFromCache(Source:BitmapData):void
		{
			for each (var key:String in _cache)
			{
				if (_cache[key] != undefined && _cache[key] == Source)
				{
					//FlxG.log ("> disposeBitmapFromCache key=" + key) ;
					_cache[key].dispose() ;
					delete _cache[key] ;
				}
			}
