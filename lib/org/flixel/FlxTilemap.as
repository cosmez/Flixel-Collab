package org.flixel
{
	...
	public class FlxTilemap extends FlxObject
	{
		...
		/**
		 * Each index of this array corresponds to a tile index, and the value corresponds to the movement cost for that tile.
		 */
		public var tileMovementCosts:Array;
		...
		protected var pathFinder:FlxPathfinding;
		...
		public function FlxTilemap()
		{
			...
			pathFinder = new FlxPathfinding(null);
		}
		 ...
		 * @param	SetupPathfinding	Whether or not to set up the FlxPathfinder for this tilemap right now.
		 * 
		 * @return	A pointer this instance of FlxTilemap, for chaining as usual :)
		 */
		public function loadMap(MapData:String, TileGraphic:Class, TileWidth:uint=0, TileHeight:uint=0, SetupPathfinding:Boolean=false):FlxTilemap
		{
			refresh = true;
			...
			if (SetupPathfinding && pathFinder == null) pathFinder = new FlxPathfinding(this);
			else
			{
				pathFinder.unload();
				if (SetupPathfinding) pathFinder.buildNodeGraph(this);
			}
		...
		public function setTileByIndex(Index:uint,Tile:uint,UpdateGraphics:Boolean=true):Boolean
		{
			...
			if (ok)
			{
				// Update the pathfinder's version of this tile.
				pathFinder.getNodeAt(Index % widthInTiles, int(Index / widthInTiles)).walkable = (Tile < collideIndex);
			}
